...
public class ComfyUIClient implements AIClient {
    ...
    private TestGenerateResult generateImageToImage(AIConfig config, TestGenerateRequest request) throws Exception {
        ...
        log.info("上传图生图输入文件: {}", imageFilename);

        // 根据工作流文件名判断默认步数：Flux2 Klein模型用4步，其他用8步
        String workflowFilename = getWorkflowFilename(config);
        int defaultSteps = (workflowFilename != null && (workflowFilename.contains("flux2") || workflowFilename.contains("klein"))) ? 4 : 8;
    ...
    /**
     * 语音图片转视频 (S2V)
     * WAN2.2 S2V 使用分块扩展模式：chunk_length=77（官方默认），batch_size=2（1个扩展子图）
     * 总帧数 = chunk_length * batch_size = 77 * 2 = 154帧 ≈ 9.6秒
     */
    private TestGenerateResult generateSoundToVideo(AIConfig config, TestGenerateRequest request) throws Exception {
        ...
        log.info("上传图片: {}, 音频: {}", imageFilename, audioFilename);

        // S2V 分块扩展模式：chunk_length 固定为77（WAN2.2官方默认）
        // batch_size=2 意味着有1个扩展子图，总帧数=77*2=154帧≈9.6秒
        // 用户可通过 frames/duration 参数控制，但实际效果取决于 batch_size
        int chunkLength = 77;  // 固定使用官方默认值
        log.info("S2V chunk_length: {} (batch_size=2, 总帧数≈154)", chunkLength);
        ...
        params.put("width", String.valueOf(request.getWidth() != null ? request.getWidth() : 640));
        params.put("height", String.valueOf(request.getHeight() != null ? request.getHeight() : 640));
        params.put("frames", String.valueOf(chunkLength));
